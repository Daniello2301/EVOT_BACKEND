...
const listarUsarios = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .send({ msg: "Error interno del servidor", error: error });
  }
};
...
const login = async (req, res) => {
  try {
    ...
    const contraseñaComparada = bcrypt.compareSync(
      req.body.contraseña,
...
const register = async (req, res) => {
  try {
    ...
    const nombreEncontrado = await Usuario.findOne({
...
const resetPassword = async (req, res) => {
  try {
    ...
    let usuarioEncontrado = await Usuario.findById({
...
const refreshUserToken = async (req, res) => {
  if (!session.usuario) {
    return res.status(401).send({ msg: "No hay usuario en sesion" });
  }
