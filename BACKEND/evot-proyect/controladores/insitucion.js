...
const crear = async (req, res) => {
    try {
        ...
        institucionGuardada = await newInstitucion.save();

        usuarioRegistrado.institucion = institucionGuardada._id;
        
        usuarioRegistrado.save();

        if(!session.usuario?.institucion){
            session.usuario.institucion = institucionGuardada._id;
        }

        return res.status(201).send(institucionGuardada);
...
const actualizar = async (req, res) => {
    try {
        ...
        const { codigoInstitucion, nombreInstitucion, ciudad, departamento, resolucion, fechaResolucion, estado } = req.body

        let institucionExistente = await Institucion.findOne({ codigoInstitucion: codigoInstitucion, _id: { $ne: id } });
        if (institucionExistente) return res.status(409).send({ msg: "La institucion ya existe" });

        institucionEncontrada.codigoInstitucion = codigoInstitucion || institucionEncontrada.codigoInstitucion;
        ...
        institucionEncontrada.usuario = institucionEncontrada.usuario;
